...
def train_validation_test_split(base_path, type_num, n_predictions, n_next, train_range, validation_range, test_range, day_range=96, norm='minmax', sum_flag=False):
    ...
    for file_name in file_names_list:
        ...
        np.save(normalization_tvt_path + 'train_x_in_%s_out_%s_range_%s.npy' % (n_predictions, n_next, train_range), train_x)
        np.save(normalization_tvt_path + 'train_y_in_%s_out_%s_range_%s.npy' % (n_predictions, n_next, train_range), train_y)
        np.save(normalization_tvt_path + 'validation_x_in_%s_out_%s_range_%s.npy' % (n_predictions, n_next, train_range), validation_x)
        np.save(normalization_tvt_path + 'validation_y_in_%s_out_%s_range_%s.npy' % (n_predictions, n_next, train_range), validation_y)
        np.save(normalization_tvt_path + 'test_x_in_%s_out_%s_range_%s.npy' % (n_predictions, n_next, train_range), test_x)
        np.save(normalization_tvt_path + 'test_y_in_%s_out_%s_range_%s.npy' % (n_predictions, n_next, train_range), test_y)
